...
import { analyzeCoaching, type CoachingInputTranscript, type CoachingOutput } from '@/lib/agentCoachingEngine';
...
export default function handler(req: NextApiRequest, res: NextApiResponse<CoachingOutput | ErrorResponse>) {
  ...
  try {
    const result = analyzeCoaching({ transcriptIds, transcripts, team, queue, flowId });
